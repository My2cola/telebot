...
async def update():
    ...
    print("last updates warzone 2.0: ", last_update)
    return last_update




@dp.message_handler(commands='warzon')
async def cmd_send_warzone(message: aiogram.types.Message):
    last_update = await update()
    await bot.send_message(
        chat_id=message.chat.id,
        text=f'last updates warzone: {last_update}',
    )
