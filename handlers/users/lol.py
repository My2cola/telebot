    ...
    while True:
        ...
        if new_update != last_update:
            ...
            await bot.send_message(
                chat_id=chat_id,
                text=f'New update for lol: {last_update}',
            )
...
async def update():
    ...
    print("last updates lol: ", last_update)
    return last_update



@dp.message_handler(commands='lol')
async def cmd_send_lol(message: aiogram.types.Message):
    last_update = await update()
    await bot.send_message(
        chat_id=message.chat.id,
        text=f'last update for lol: {last_update}',
    )
