    ...
    while True:
        ...
        if new_update != last_update:
            ...
            await bot.send_message(
                chat_id=chat_id,
                text=f'New update for fall guys: {last_update}',
            )
...
async def update():
    ...
    print("last updates among_us: ", last_update)
    return last_update




@dp.message_handler(commands='among_us')
async def cmd_send_among_us(message: aiogram.types.Message):
    last_update = await update()
    await bot.send_message(
        chat_id=message.chat.id,
        text=f'last update for among_us: {last_update}',
    )
